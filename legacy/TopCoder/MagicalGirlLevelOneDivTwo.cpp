...
public:
   double theMinDistance( int d, int x, int y ) 
   {
	   ...
	   YS = ( y < 0 ? -1 : 1 );
	   int XRes , YRes;
	   for ( XRes = 0 ; ( XS == 1 && XRes + d < x ) || ( XS == -1 && XRes - d > x ) ; XRes += XS );
	   for ( YRes = 0 ; ( YS == 1 && YRes + d < y ) || ( YS == -1 && YRes - d > y ) ; YRes += YS );
	   double Result = sqrt( pow( (double)XRes , 2 ) + pow( (double)YRes , 2 ) );
	   return Result;
   }
};
...
namespace moj_harness {
	...
	int run_test_case( int casenum ) {
		...
		case 0: {
			...
			double received           = MagicalGirlLevelOneDivTwo().theMinDistance( d, x, y );
