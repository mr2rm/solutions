...
public:
   double expectedPoints( int N, vector <int> ringPoints ) 
   {
	   ...
	   {
		   Temp = (double)( 2 * i + 1 ) / pow( (double)N + 1 , 2 );
		   Result += ( Temp * ringPoints[i] );
	   }
	   return Result;
   }
};
...
namespace moj_harness {
	...
	int run_test_case( int casenum ) {
		...
		case 0: {
			...
			double received           = Archery().expectedPoints( N, vector <int>( ringPoints, ringPoints + ( sizeof ringPoints / sizeof ringPoints[0] ) ) );
