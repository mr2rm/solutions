...
def list_final_submissions(contest_id):
    return Submission.objects.filter(problem__contest__id=contest_id).values('participant_id', 'problem_id').annotate(
        Max('score')).annotate(Max('submitted_time')).order_by('-submitted_time__max').values('participant_id',
                                                                                             'problem_id', 'score__max')
